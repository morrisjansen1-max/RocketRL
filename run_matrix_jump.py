"""Single-jump baselines: PPO and SAC, no intermediate staging.

Both algorithms warm-start from their from-scratch phase-1 policy and then take a
single fine-tune straight to the full environment (gimbal + RCS + mass depletion
+ drag + unobserved wind), skipping the gimbal and mass stages. This is the
symmetric counterpart to run_matrix_staged.py's staged curricula: comparing these jump
policies against the staged ones shows how each algorithm handles a single large
distribution shift versus a gradual one.

  PPO jump: phase1_ppo -> phase2_ppo
  SAC jump: phase1_sac -> phase2_sac

DEPENDENCY: both fine-tune from the phase-1 stages, so run run_matrix_staged.py
first (it trains phase1_ppo and phase1_sac, the init_from parents).

Roughly 1.5 h on an M-series laptop CPU (the SAC jump dominates; the PPO jump is
~20 min). Resumable: a step is skipped when its completion artifact exists.
...
JUMP = [
    ("configs/phase2_ppo.yaml", "phase2_ppo", 10),
    ("configs/phase2_sac.yaml", "phase2_sac", 10),
]
